...
async def button_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ================== OWNERSHIP CONFIRM (SELLER) ==================
    if query.data.startswith("confirm_ownership_"):
        group_id = query.data.split("_")[-1]
        group = data["groups"][group_id]

        if str(query.from_user.id) != str(group["seller_id"]):
            await query.answer("❌ You are not the seller of this group.")
            return

        group["ownership_status"] = "transferred"
        save_data(data)

        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=(
                f"👤 Seller confirmed ownership transfer.\n\n"
                f"Group: {group['link']}\n"
                f"Target ID: {group['ownership_target_id']}\n\n"
                "Please verify the transfer and mark as completed manually."
            )
        )
        await query.edit_message_text("✅ Ownership transfer confirmed. Waiting for admin verification.")
        return

    # ================== VERIFY OWNERSHIP (ADMIN) ==================
    if query.data.startswith("verify_ownership_"):
        gid = query.data.split("_")[-1]
        group = data["groups"][gid]
        group["ownership_status"] = "verified"
        save_data(data)

        await query.edit_message_text(f"✅ Ownership verified for group {group['link']}.")
        await context.bot.send_message(
            chat_id=group["seller_id"],
            text="🎉 Your group ownership has been successfully verified by admin."
        )
        return

# ========================
# HANDLE MESSAGE (Group link / Ownership Target)
# ========================
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ================= SELLER SENDING GROUP LINK =================
    if context.user_data.get("awaiting_group_link"):
        context.user_data.pop("awaiting_group_link")
        ...
        # Notify admin
        await update.message.reply_text("✅ Your group has been submitted for approval.")
        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=(
                f"📢 New group submitted by @{update.effective_user.username or user_id}\n"
                f"Link: {text}\n\n"
                f"Approve or Reject?"
            ),
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Approve", callback_data=f"admin_approve_{group_id}")],
                [InlineKeyboardButton("❌ Reject", callback_data=f"admin_reject_{group_id}")]
            ])
        )
        return

    # ================= ADMIN SENDING OWNERSHIP TARGET ID =================
    if context.user_data.get("awaiting_ownership_id"):
        info = context.user_data.pop("awaiting_ownership_id")
        target_id = text

        group = data["groups"][info["group_id"]]
        group["ownership_status"] = "requested"
        group["ownership_target_id"] = target_id
        save_data(data)
        ...
        await context.bot.send_message(
            ...
            text=(
                ...
                "After you complete the transfer, click the button below 👇"
            ),
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Ownership Transferred", callback_data=f"confirm_ownership_{info['group_id']}")]
            ])
        )
...
async def admin_approval_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not str(query.from_user.id) == str(ADMIN_ID):
        await query.answer("❌ Only admin can do this.")
        return
    ...
    if action == "approve":
        ...
        save_data(data)
        await query.edit_message_text(f"✅ Group approved.\n\n{group['link']}")

        # Ask admin to provide target ID
        await query.message.reply_text(
            f"✅ Group approved.\n\nNow send me the Telegram @username or ID "
            f"to which the seller should transfer the ownership of:\n\n{group['link']}"
        )
        context.user_data["awaiting_ownership_id"] = {
            "seller_id": seller_id,
            "group_id": gid
        }
        ...
        await context.bot.send_message(
            chat_id=seller_id,
            text="✅ Your group has been approved. Admin will send you ownership transfer details soon."
        )
    else:
        group["group_status"] = "rejected"
        save_data(data)
        await query.edit_message_text(f"❌ Group rejected.\n\n{group['link']}")
        await context.bot.send_message(
            chat_id=seller_id,
            text="❌ Your group submission was rejected."
        )
